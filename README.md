...
## Camera selection (CSI + USB)
The server enumerates the connected `/dev/video*` capture nodes at startup and picks a
source based on each device's V4L2 driver:
- **CSI** sensors (`tegra-video` on Jetson) stream through `nvarguscamerasrc` (Jetson) or
  `libcamerasrc` (Pi).
- **USB / UVC** webcams (`uvcvideo`) stream through `v4l2src`, with `decodebin` so both
  MJPEG and raw-YUYV cameras work.

By default it auto-selects the **lowest-numbered** capture device (e.g. `/dev/video0`),
so on a board with both a CSI and a USB camera the CSI sensor (which comes up as
`video0`) wins. To force a specific camera, set `device` under `[camera]` in
`config.toml` (e.g. `device = "/dev/video1"`); the ARK-OS web UI writes this field when
you choose a camera on the Video page. UVC metadata-only nodes are filtered out, so they
are never selected.
